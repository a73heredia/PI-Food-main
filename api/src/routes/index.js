...
const getRecipesInfo = async () => {
    const get = await axios.get('https://api.spoonacular.com/recipes/complexSearch?apiKey=2d8d5b6ff79b48ddbc03e7ba836f253e&addRecipeInformation=true&number=100');
...
router.get('/recipes', async(req, res) => {
    ...
    if(name){
        let recipeName = await recipes.filter(el => el.name.toLowerCase().includes(name.toLowerCase()));
...
router.get('/diets', async(req, res) => {
    ...
    const allData = await axios.get('https://api.spoonacular.com/recipes/complexSearch?apiKey=2d8d5b6ff79b48ddbc03e7ba836f253e&addRecipeInformation=true&number=100');
